...
jQuery(function($) {
	...
	$.fn.nyroModal.settings = {
		...
		padding: 35, // padding for the max modal size
	...
	function clickBg(e) {
        // nyroModalContentの子を取得する
        var modal_content = $('#nyroModalContent').children();
        modal_content = modal_content[0];
		if (!currentSettings.modal) {
            // kcwのみ削除確認ダイアログを出力
            if (modal_content.id == 'kcw') {
                removeModal('',true);
            } else {
                // moda通常終了
                removeModal();
	...
	function removeModal(e,is_confirm) {
        if (is_confirm == undefined) { 
            is_confirm = false; 
        }
        if(is_confirm) {
            rObj = new RegExp('jpn-JP');
            if(!modal.content[0].baseURI.match(rObj)) {
                if (!window.confirm('Are you sure you want to close the modal ?')) {
                    return false;
                }
            } else {
                if (!window.confirm('閉じてもよろしいですか ?')) {
                    return false;
                }
            }
        }
