    ...
    <div class="context-toolbar">
        <div class="button-left">
            <p class="table-preferences">
                ...
                {else}
                    <a title="{'Show 10 items per page.'|i18n( 'design/admin/node/view/full' )}" href={concat('kaltura/gallery_media/', $page, '/10/', $view_mode)|ezurl}>10</a>
                {/if}
                {if eq($page_size, 25)}
                    <span class="current">25</span>
                {else}
                    <a title="{'Show 25 items per page.'|i18n( 'design/admin/node/view/full' )}" href={concat('kaltura/gallery_media/', $page, '/25/', $view_mode)|ezurl}>25</a>
                {/if}
                {if eq($page_size, 50)}
                    <span class="current">50</span>
                {else}
                    <a title="{'Show 50 items per page.'|i18n( 'design/admin/node/view/full' )}" href={concat('kaltura/gallery_media/', $page, '/50/', $view_mode)|ezurl}>50</a>
                {/if}
            </p>
        </div>
        <div class="button-right">
            <p class="table-preferences">
                ...
                {else}
                    <a title="{'Display sub items as thumbnails'|i18n('design/admin/content/browse')}" href={concat('kaltura/gallery_media/', $page, '/',$page_size,'/1')|ezurl}>{'Thumbnail'|i18n( 'design/admin/node/view/full' )}</a>
                {/if}
                {if eq($view_mode, 2)}
                    <span class="current">{'Detailed'|i18n( 'design/admin/node/view/full' )}</span>
                {else}
                    <a title="{'Display sub items using a detailed list.'|i18n( 'design/admin/node/view/full' )}" href={concat('kaltura/gallery_media/', $page, '/',$page_size,'/2')|ezurl}>{'Detailed'|i18n( 'design/admin/node/view/full' )}</a>
                {/if}
    ...
    <div style="margin: 20px 0px 30px 0px;">
        ...
        {if eq($view_mode, 1)}
            ...
            <div class="list-thumbnails-item">
                ...
                <div class="controls">
                    <p>
                        {$kaltura.status|i18n( 'ezkaltura/status' )}
                    </p>
                    <p>
                        {$kaltura.name|shorten( 23, '...' )}
                    </p>
                    {if gt(count($kaltura.ezkaltura),0)}
                    <p>
        ...
        <table cellspacing="0" class="list">
            ...
            <tr {if eq($num,2)}style="background-color: #f5f5f5;"{/if}>
                ...
                <td style="vertical-align: middle;">{$kaltura.mediaType|i18n( 'ezkaltura/mediatype' )}</td>
                <td style="vertical-align: middle;">
                    {if ne($kaltura.mediaType,'AUDIO')}
                        {if and($kaltura.width, $kaltura.height)}
                            {$kaltura.width} x {$kaltura.height}
                        {/if}
                    {else}
                        {'No Data'|i18n( 'ezkaltura' )}
                    {/if}
        ...
        <div class="pagenavigator" style="margin-top: 20px;">
            ...
            {def $dots    = false()}
            <p>
                <span class="previous">{if gt($page, 1)}<a href={concat('kaltura/gallery_media/',sub($page, 1), '/', $page_size,'/',$view_mode)|ezurl}">{/if}<span {if eq($page, 1)}class="text disabled"{/if}>≪&nbsp;前へ</span>{if gt($page, 1)}</a>{/if}</span>
                    ...
                    {for 1 to $total as $i}
                        ...
                        {else}
                            {if or( le($i, $endSize), and($page, ge($i, sub($page, $midSize)), le($i, sum($page, $midSize))), gt($i, sub($total, $endSize)))}
                                <span class="other"><a href={concat('kaltura/gallery_media/', $i, '/', $page_size,'/',$view_mode)|ezurl}>{$i}</a></span>
                                {set $dots = true()}
                            {elseif $dots}
...
<!--
	...
	function doDelete(contentobject_attribute_id, entry_id, version){
        location.href = 'http://{/literal}{concat($server_data.env_data,'/',$server_data.current_site_access)}{literal}/kaltura/delete?entry_id=' + entry_id + '&contentobject_attribute_id=' + contentobject_attribute_id + '&version=' + version + '&is_delete_other_version=1&media_type=media';
	}
	// 削除確認する
