...
{
    // kalturaからデータを取得
    $response = $ez_kaltura_comm->client->media->listAction($filter, $pager);
...
$Result['content']   = $tpl->fetch( 'design:kaltura/gallery_media.tpl');
$Result['path']      = array( array( 'url' => false,
                                        'text' => 'Kaltura GalleryMedia' ) );
...
function getMaxVersion($data) {
    ...
    if(count($data) > 0) {
        foreach($data as $value) {
            ...
            // キーが存在するかどうか
            if (!array_key_exists($entry_id, $return)) {
